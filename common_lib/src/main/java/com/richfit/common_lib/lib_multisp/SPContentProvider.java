...
import static com.richfit.common_lib.lib_multisp.ConstantUtil.TYPE_GET_ALL;
...
public class SPContentProvider extends ContentProvider {
            ...
            if (all != null) {
                ...
                for (String key : keySet) {
                    ...
                    if (rows[2] instanceof Boolean) {
                        rows[1] = ConstantUtil.TYPE_BOOLEAN;
                    } else if (rows[2] instanceof String) {
                        rows[1] = ConstantUtil.TYPE_STRING;
                    } else if (rows[2] instanceof Integer) {
                        rows[1] = ConstantUtil.TYPE_INT;
                    } else if (rows[2] instanceof Long) {
                        rows[1] = ConstantUtil.TYPE_LONG;
                    } else if (rows[2] instanceof Float) {
                        rows[1] = ConstantUtil.TYPE_FLOAT;
                    }
                    cursor.addRow(rows);
                }
                return cursor;
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        String type = path[1];
        String key = path[2];
        Object obj = (Object) values.get(ConstantUtil.VALUE);
        if (obj != null)
