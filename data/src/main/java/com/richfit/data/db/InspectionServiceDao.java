...
public class InspectionServiceDao extends BaseDao implements IInspectionServiceDao {
    ...
    public InspectionServiceDao( Context context) {
        super(context);
    }
    ...
    public void saveTakedImages(ArrayList<ImageEntity> images, String refNum, String refLineId, int takePhotoType, String imageDir, boolean isLocal) {
        ...
        for (ImageEntity image : images) {
            ...
            cv.put("creation_date", CommonUtil.transferLongToDate("yyyyMMddHHmmss", image.lastModifiedTime));
    ...
    private String saveInspectionHeader(SQLiteDatabase db, ResultEntity param) {
        ...
        final long creationDate = CommonUtil.getSystemDate();
        final String currentDate = CommonUtil.getCurrentDate(Global.GLOBAL_DATE_PATTERN_TYPE1);
        long iResult = -1;
        ContentValues cv = new ContentValues();
        if (TextUtils.isEmpty(insId)) {
            //如果为空那么插入一条
            insId = CommonUtil.getUUID();
            cv.put("id", insId);
